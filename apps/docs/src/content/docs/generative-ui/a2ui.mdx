...
Every workspace gives the model a built-in **`render_ui`** tool (turn it off per workspace under **Settings → Workspaces → Generative UI**). Its description teaches the model the component catalog, so it works with any tool-calling model — GPT, Claude, Gemini, Llama.
...
Moka validates the tree (a `root` component must exist; every `child`/`children` reference must resolve). If it's invalid, the model gets a precise error and usually fixes it on the next step.
</TabItem>
